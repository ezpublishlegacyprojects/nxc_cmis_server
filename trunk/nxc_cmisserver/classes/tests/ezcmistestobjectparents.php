<?php
/**
 * Definition of eZCMISTestObjectParents class
 *
...
/**
 * Tests fetching of object parents
 *
 * @file ezcmistestobjectparents.php
 */
...
class eZCMISTestObjectParents extends eZCMISTestBase
{
    /**
     * @reimp
     */
    protected function test()
    {
        ...
        $title = 'TestObjectParents title ' . rand();
        ...
        $parents = eZCMISAtomTools::processXML( $result->response, '/atom:entry/atom:link[@rel="parents"]' );
        $parentsLink = isset( $parents[0]['href'] ) ? $parents[0]['href'] : false;
        ...
        $result = $this->httpRequest( $parentsLink, $this->User, $this->Password, array(), 'GET', array() );

        $this->checkCode( '200', $result->code, 'GET', $parentsLink, $result->response );
        $this->checkResponse( $result->response );
        ...
        if ( !$objectId )
        {
            $this->addMessage( ezi18n( 'cmis', 'Could not fetch "%name%"', null, array( '%name%' => 'ObjectId' ) ) );
            $this->addMessage( $result->response );
            $this->throwError();
        }
    }
