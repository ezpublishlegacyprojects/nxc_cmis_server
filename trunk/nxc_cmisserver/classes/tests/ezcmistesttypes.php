<?php
/**
 * Definition of eZCMISTestTypes class
 *
...
/**
 * Tests fetching of all types
 *
 * @file ezcmistesttypes.php
 */
...
class eZCMISTestTypes extends eZCMISTestBase
{
    /**
     * @reimp
     */
    protected function test()
    {
        ...
        $descendantslinkDom = eZCMISAtomTools::processXML( $result->response, '/app:service/app:workspace/app:collection[@cmis:collectionType="typesdescendants"]' );
        $descendantsLink = isset( $descendantslinkDom[0]['href'] ) ? (string) $descendantslinkDom[0]['href'] : false;

        if ( !$descendantsLink )
        {
            $this->addMessage( ezi18n( 'cmis', 'Could not fetch "%collection%" collection', null, array( 'typesdescendants' ) ) );
